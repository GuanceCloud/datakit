...
func B64decChecking(_ *runtime.Context, funcExpr *ast.CallExpr) error {
...
func B64dec(ctx *runtime.Context, funcExpr *ast.CallExpr) runtime.PlPanic {
	if len(funcExpr.Param) != 1 {
		return fmt.Errorf("func %s expects 1 arg", funcExpr.Name)
	}
	if funcExpr.Param[0].NodeType != ast.TypeIdentifier {
	...
	if err := ctx.AddKey2PtWithVal(key, string(res), ast.String,
