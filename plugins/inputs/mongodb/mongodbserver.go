...
func (s *Server) gatherCollectionStats(colStatsDbs []string) (*ColStats, error) {
	...
	for _, dbName := range names {
		if stringInSlice(dbName, colStatsDbs) || len(colStatsDbs) == 0 {
			...
			if err != nil {
				l.Errorf("Error getting collection names: %s", err.Error())
				continue
			}
...
func (s *Server) gatherData(gatherClusterStatus bool, gatherDbStats bool, gatherColStats bool, gatherTopStat bool, colStatsDbs []string) error {
	...
	if gatherClusterStatus {
		...
		if err != nil {
			l.Debugf("Unable to gather cluster status: %s", err.Error())
		}
	...
	if gatherDbStats {
		...
		for _, name := range names {
			...
			if err != nil {
				l.Debugf("Error getting db stats from %q: %s", name, err.Error())
			}
