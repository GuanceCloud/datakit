...
func TestFormatPacketV1Generic(t *testing.T) {
	...
	assert.Equal(t, "snmp-traps", trapContent["agent_source"])
	assert.Equal(t, "snmp_version:1,device_namespace:totoro,snmp_device:127.0.0.1", trapContent["agent_tags"])
...
func TestFormatPacketToJSON(t *testing.T) {
	...
	assert.Equal(t, "snmp-traps", trapContent["agent_source"])
	assert.Equal(t, "snmp_version:2,device_namespace:totoro,snmp_device:127.0.0.1", trapContent["agent_tags"])
...
func TestNewJSONFormatterWithNilStillWorks(t *testing.T) {
	formatter, err := NewJSONFormatter(NoOpOIDResolver{}, "mononoke")
	require.NoError(t, err)
	packet := createTestPacket(NetSNMPExampleHeartbeatNotification)
	...
	data := []struct {
	...
	}{
		{
			...
			expectedContent: map[string]interface{}{
				...
				"agent_tags":                  "snmp_version:2,device_namespace:totoro,snmp_device:127.0.0.1",
				"timestamp":                   0.,
				"uptime":                      float64(1000),
		...
		},
		{
			...
			expectedContent: map[string]interface{}{
				"agent_source":  "snmp-traps",
				"agent_tags":    "snmp_version:2,device_namespace:mononoke,snmp_device:127.0.0.1",
				"timestamp":     0.,
				"snmpTrapName":  "linkUp",
				"snmpTrapMIB":   "IF-MIB",
		...
		},
		{
			...
			expectedContent: map[string]interface{}{
				"agent_source":  "snmp-traps",
				"agent_tags":    "snmp_version:2,device_namespace:sosuke,snmp_device:127.0.0.1",
				"timestamp":     0.,
				"snmpTrapName":  "linkUp",
				"snmpTrapMIB":   "IF-MIB",
				"snmpTrapOID":   "1.3.6.1.6.3.1.1.5.4",
		...
		},
		{
			...
			expectedContent: map[string]interface{}{
				"agent_source":  "snmp-traps",
				"agent_tags":    "snmp_version:2,device_namespace:nausicaa,snmp_device:127.0.0.1",
				"timestamp":     0.,
				"snmpTrapName":  "linkUp",
				"snmpTrapMIB":   "IF-MIB",
				"snmpTrapOID":   "1.3.6.1.6.3.1.1.5.4",
...
func TestFormatterWithResolverAndTrapV1Generic(t *testing.T) {
	...
	assert.Equal(t, "snmp-traps", trapContent["agent_source"])
	assert.Equal(t, "snmp_version:1,device_namespace:porco_rosso,snmp_device:127.0.0.1", trapContent["agent_tags"])
...
func TestIsValidOID_PropertyBasedTesting(t *testing.T) {
	...
	invalidRunes := []rune(",?><|\\}{[]()*&^%$#@!abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
	for i := 0; i < testSize; i++ {
		// Valid cases
		oid := validOIDs[i]
		x := 0
