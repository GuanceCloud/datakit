...
func TestConnMerge(t *testing.T) {
	...
	expected := map[ConnectionInfo]ConnFullStats{
		{
			Saddr: [4]uint32{0, 0, 0, 0x01},
			Sport: 1000,
			Daddr: [4]uint32{0, 0, 0, 0x01},
	...
	MergeConns(&connResultOrigin)
	if len(expected) != len(connResultOrigin.result) {
		t.Error("len not equal", len(expected), " ", len(connResultOrigin.result))
	}
	for k, v := range expected {
		if vp, ok := connResultOrigin.result[k]; !ok {
