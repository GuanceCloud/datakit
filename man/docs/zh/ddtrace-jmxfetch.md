...
默认会采集 JVM 信息：JVM CPU、JVM MEM、JVM Thread、class等。 具体[指标集列表列表](./jvm/#dd-jvm-measurement){:target="_blank"}
...
## 如何通过自定义配置采集指标 {#custom_metric}

### 1 自定义 JVM 线程状态指标 `jvm.total_thread_count` `jvm.peak_thread_count` `jvm.daemon_thread_count` {#custom_jvm}
