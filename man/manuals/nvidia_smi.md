...
NVIDIA-SMI 指标展示：包括 NVIDIA GPU 卡温度、时钟、GPU占用率、内存占用率、GPU内每个运行程序的内存占用 等。
...
| 环境变量名                        | 对应的配置参数项 | 参数示例                                                     |
|:-----------------------------| ---              | ---                                                          |
| `ENV_INPUT_NVIDIASMI_TAGS`   | `tags`           | `tag1=value1,tag2=value2` 如果配置文件中有同名 tag，会覆盖它 |
| `ENV_INPUT_NVIDIASMI_INTERVAL` | `interval`       | `10s`                                                        |
...
NVIDIA-DCGM 指标展示：包括 NVIDIA GPU 卡温度、时钟、GPU占用率、内存占用率 等。
...
```
 ...
 [指标] 选 [日志] -> [指标集] 选 [nvidia_smi] -> 第4栏选 [status_gpu] -> 第3栏选 [Max] -> by[检测维度] 选 [host]+[uuid]
