...
- [v]支持[通过 Git 来管理]() datakit 的各种 conf（`datakit.conf` 除外）以及 Pipeline(#366)
- [v]支持[全离线安装]()(#421)
- [v]eBPF-network
	- 增加[DNS 数据采集]()(#418)
	- 增强内核适配性，内核版本要求已降低至 Linux 4.4+(#416)
- [v]增强数据调试功能，采集到的数据支持写入本地文件，同时发送到中心(#415)
- [v]K8s 环境中，默认开启的采集器支持通过环境变量注入 tags，详见各个默认开启的采集器文档(#408)
- [v]DataKit 支持[一键上传日志]()(#405)
- [v]MySQL 采集器增加[SQL 语句执行性能指标]()(#382)
- [v]修复安装脚本中 root 用户设定的 bug(#430)
- [v]增强 Kubernetes 采集器：
	- [v]增强 Annotation 在采集器配置中的作用(#380)
	- [v]增加更多 Annotation key，支持多 IP 情况(#419)
	- [v]支持采集 Node IP(#411)
- [v]云同步增加华为云与微软云支持(#265)
