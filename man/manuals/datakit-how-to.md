...
- `gin.log`：DataKit 可以接收外部的 HTTP 数据输入，这个日志文件相当于 HTTP 的 access-log（DataKit 日志需[开启 `debug` 选项](#config-logging)才能看到 gin.log，否则 gin.log 内容为空）
- `log`：DataKit 运行日志
...
具体参见 [monitor 使用](datakit-monitor.md)。
