...
pipeline 文件的加载是按对应 conf 文件中的 `pipeline` 来进行加载的。
- 如果这个值是一个<strong>文件名</strong>，则会加载 `pipeline` 目录下的指定的文件.
- 如果这个值是一个<strong>路径</strong>，则会加载 `gitrepos` 目录下指定的文件。
...
[[inputs.nginx]]
    ...
    [inputs.nginx.log]
    ...
    pipeline = "myconfs/nginx.p" # 对应加载 gitrepos/myconfs/nginx.p 文件
    # pipeline = "nginx.p" # 对应加载 pipeline/nginx.p 文件
    ...
```
