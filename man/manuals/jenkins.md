...
- 确保在配置文件中开启了 Jenkins CI Visibility 功能，且配置了监听端口号；
- 在 Jenkins 中安装 [Jenkins Datadog plugin](https://plugins.jenkins.io/datadog/) ；
- 在 Manage Jenkins > Configure System > Datadog Plugin 中选择 `Use the Datadog Agent to report to Datadog (recommended)`，配置 `Agent Host` 为 Datakit IP 地址，`DogStatsD Port` 及 `Traces Collection Port` 两项均配置为 Jenkins 配置文件中配置的端口号，如 `9539`；
