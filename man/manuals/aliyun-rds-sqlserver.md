...
- 通过一台服务器采集所有阿里云 RDS SQLServer 数据
...
![image](imgs/input-rds-sqlserver-5.png)

3. 载入阿里云数据同步脚本，管理 - 脚本市场 - 阿里云数据同步 (云监控)

![image](imgs/input-rds-sqlserver-6.png)

#### 添加脚本

1. 阿里云数据同步 (云监控) - 添加脚本

![image](imgs/input-rds-sqlserver-7.png)
...
![image](imgs/input-rds-sqlserver-9.png)

#### 定时任务

1. 添加自动触发任务，管理 - 自动触发配置 - 新建任务

![image](imgs/input-rds-sqlserver-10.png)

2. 自动触发配置，执行函数中添加此脚本，其他默认即可

![image](imgs/input-rds-sqlserver-11.png)

3. 指标预览

![image](imgs/input-rds-sqlserver-12.png)
...
<监控 - 模板新建 - 阿里云 RDS SQLServer 检测库>

## 指标详解

<[阿里云 RDS SQLServer 指标列表](https://help.aliyun.com/document_detail/214180.html)>
