...
| `query`                  | DQL查询语句（DQL [文档]()）                                                                                                                |
| `conditions`             | 额外添加条件表达式，使用 DQL 语法，与现有 `query` 中的条件表达式成 `AND` 关系，且会在最外层添加括号避免与其混乱                            |
...
| `orderby`                | 指定`order by`参数，内容格式为 `map[string]string` 的数组，例如 `"asc":"name"` 或 `"desc":"age"`，此条会替换原查询语句中的 `order by`      |
