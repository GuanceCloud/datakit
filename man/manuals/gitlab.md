...
1. 在配置文件中，配置 `enable_ci_visibility = true` ，重启 Datakit，让 Datakit 在 http://Datakit_IP:PORT/v1/write/gitlab 监听 Gitlab Webhook 发送的 event；
2. 在 Gitlab 转到 `Settings` > `Webhooks` 中，将 URL 配置为 http://Datakit_IP:PORT/v1/write/gitlab，Trigger配置 Job events 和 Pipeline events 两项，点击 Add webhook 确认添加；
