...
![image](imgs/input-aliyun-pstgresql-2.png)

3、保存或下载 **AccessKey** **ID** 和 **AccessKey Secret** 的 CSV 文件 (配置文件会用到)
4、用户授权 (云监控只读/时序指标数据权限)

![image](imgs/input-aliyun-pstgresql-3.png)
...
- 通过一台服务器采集所有阿里云 PolarDB PostgreSQL 数据
...
![image](imgs/input-aliyun-pstgresql-4.png)

2、开启脚本市场，管理 - 实验性功能 - 开启脚本市场模块

![image](imgs/input-aliyun-pstgresql-5.png)

3、载入阿里云数据同步脚本，管理 - 脚本市场 - 阿里云数据同步 (云监控)

![image](imgs/input-aliyun-pstgresql-6.png)

#### 添加脚本

1、阿里云数据同步 (云监控) - 添加脚本

![image](imgs/input-aliyun-pstgresql-7.png)

2、输入标题/描述信息

![image](imgs/input-aliyun-pstgresql-8.png)
...
![image](imgs/input-aliyun-pstgresql-9.png)

#### 定时任务

1、添加自动触发任务，管理 - 自动触发配置 - 新建任务

![image](imgs/input-aliyun-pstgresql-10.png)

2、自动触发配置，执行函数中添加此脚本，其他默认即可

![image](imgs/input-aliyun-pstgresql-11.png)

3、指标预览

![image](imgs/input-aliyun-pstgresql-12.png)

## 场景视图

<场景 - 新建仪表板 - 内置模板库 - 阿里云 PolarDB PostgreSQL>
