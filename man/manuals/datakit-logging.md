...
> 这里建议使用通配路径（设置可以配置当前不存在、但将来会冒出来的文件），而不是将日志路径写死，因为应用的日志可能不会立即出现（比如部分应用的 error log 只有 error 发生的时候才会出现）。

磁盘文件采集有一点需要注意，即它只会采集==自 DataKit 启动后有更新的日志文件==，如果配置的日志文件（自 DataKit 启动后）没有更新，其==历史数据是不会采集的==。

正因为这个特性，如果日志文件持续在更新，如果中间停止 DataKit，==该空窗期的日志也不会被采集到==，后面可能会做一些策略来缓解这个问题。
...
- 由于部署在容器环境中的应用，其均需要构建对应的应用镜像。对 DataKit 而言，可以基于镜像名称，选择性的针对某些应用做日志采集。
...
对远程日志推送而言，其主要是开发者主动将日志推送到 DataKit 指定的服务上，比如 [Java 的 log4j](logging_socket#java) 以及 [Python 原生的 `logging.handler.SocketHandler`](logging_socket#Python) 均支持将日志发送给远端服务。

这种形式的特点是日志直接发送给 DataKit，中间无需落盘（上面的 Docker API 方式采集日志实际上还是有落盘的）。这种形式的日志采集，需注意以下几点：
