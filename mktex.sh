#!/bin/bash
# author: tanbiao
# date: Wed Oct 19 19:21:10 CST 2022
#
# This tool used to generate datakit related docs to  latex
#
...
tex_dir=.tex
tmp_doc_dir=.docs
datakit_docs_dir=${tex_dir}/docs/datakit
developers_docs_dir=${tex_dir}/docs/developers
pwd=$(pwd)
...
for f in "${datakit_docs[@]}"; do
	#tex_name=${f%.bash}.tex
	tex_name=`basename $f`
	kramdown -o latex --template document $f > $datakit_docs_dir/$tex_name.tex
done
...
for f in "${developers_docs[@]}"; do
	#tex_name=${f%.bash}.tex
	tex_name=`basename $f`
	kramdown -o latex --template document $f > `basename $developers_docs_dir/$tex_name.tex
done
