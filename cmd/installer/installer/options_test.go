...
func TestSetupService(t *T.T) {
	...
	t.Run(`no-cpu-mem-limit:use-default`, func(t *T.T) {
		...
		assert.Equal(t, fmt.Sprintf("%.0f%%", rl.CPUCores*100), scfg.Option["CPUQuota"])
	...
	t.Run(`cpu-mem-limit:use-args`, func(t *T.T) {
		...
		assert.Equal(t, fmt.Sprintf("%.0f%%", args.LimitCPUMax*float64(runtime.NumCPU())), scfg.Option["CPUQuota"])
