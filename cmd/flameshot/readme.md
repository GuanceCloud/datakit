...
| `FLAMESHOT_PROFILING_PATH` | Yes | `/data` | Shared writable directory for profiler tools, JFR output, and OOM related files |
...
- If the Pod is killed too quickly, `.hprof` may still fail to appear.
...
2. If an `oom_kill` increment is later observed, try to locate the matching `.hprof` and upload an OOM summary log.
...
4. Check whether `FLAMESHOT_PROFILING_PATH` is shared and writable.
