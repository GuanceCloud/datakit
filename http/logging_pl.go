...
func (std *logTaskData) callback(task *plw.Task, result []*pipeline.Result) (*plw.Task, []*pipeline.Result, error) {
	result = plw.ResultUtilsLoggingProcessor(task, result, nil, nil)
	if len(result) != len(std.point) {
		return nil, nil, fmt.Errorf("result count is less than input")
	}
...
func (std *logTaskData) Callback(task *plw.Task, result []*pipeline.Result) error {
	task, result, err := std.callback(task, result)
	if err != nil {
		return err
	}
