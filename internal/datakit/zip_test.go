...
func BenchmarkUnGzipString(b *T.B) {
	data := cliutils.CreateRandomString(1024)
	zdata, err := GZip([]byte(data))
...
func BenchmarkGzipString(b *T.B) {
	data := cliutils.CreateRandomString(1024)

	b.ResetTimer()
	b.Run("no-pool", func(b *T.B) {
		for i := 0; i < b.N; i++ {
			_, err := GZipStr(data)
			assert.NoError(b, err)
		}
	...
	b.Run("pooled", func(b *T.B) {
		for i := 0; i < b.N; i++ {
			_, err := GZipStrV2(data)
			assert.NoError(b, err)
		}
	})
}

func BenchmarkGzipBytes(b *T.B) {
	data := cliutils.CreateRandomString(1024)
	for i := 0; i < b.N; i++ {
...
func TestZipRW(t *T.T) {
	t.Run("zip-string", func(t *T.T) {
		data := cliutils.CreateRandomString(1024)
		zdata, err := GZipStrV2(data)
		assert.NoError(t, err)
		assert.NotNil(t, zdata)

		t.Logf("len(zdata): %d", len(zdata))
