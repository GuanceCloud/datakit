...
func TestJITNestedControlUpstreamOracle(t *testing.T) {
	...
	cases := []struct {
		name                   string
		source                 string
		dropped, branch, after bool
	}{
		{name: "drop_argument", source: `add_key(before,true); add_key(result,drop()); add_key(after,true)`, dropped: true, after: true},
		{name: "exit_argument", source: `add_key(before,true); add_key(result,exit()); add_key(after,true)`},
		{name: "drop_condition", source: `add_key(before,true); if drop()==nil { add_key(branch,true) }; add_key(after,true)`, dropped: true, branch: true, after: true},
		{name: "exit_condition", source: `add_key(before,true); if exit()==nil { add_key(branch,true) }; add_key(after,true)`, branch: true},
	}
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			expected := newRealScriptPoint("nested-control", map[string]any{"message": "test"})
			run := pointRun{point: expected, script: script, started: time.Now()}
			runPipelineGo(point.Logging, &run, nil)
			if run.dropped != test.dropped || expected.Get("before") != true ||
				(expected.Get("branch") != nil) != test.branch || (expected.Get("after") != nil) != test.after {
				t.Fatalf("unexpected pipeline-go oracle dropped=%v point=%s", run.dropped, expected.Pretty())
			}

			if check := runner.Check(test.source); check.Route != pljit.RouteJITNative {
				t.Fatalf("nested control did not route native: %#v", check)
			}
			actual := newRealScriptPoint("nested-control", map[string]any{"message": "test"})
