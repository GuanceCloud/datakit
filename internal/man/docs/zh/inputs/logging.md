...
[Pipeline](../developers/pipeline/) 主要用于切割非结构化的文本数据，或者用于从结构化的文本中（如 JSON）提取部分信息。
...
??? info
    ...
    此处 `*2` 考虑到了 [Pipeline 切割](../developers/pipeline/)导致的实际数据膨胀，而一般情况下，切割完都是要带上原始数据的，故按照最坏情况考虑，此处以加倍方式来计算。
<!-- markdownlint-enable -->
