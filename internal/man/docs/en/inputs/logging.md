...
[Pipeline](../developers/pipeline/) is used primarily to cut unstructured text data, or to extract parts of information from structured text, such as JSON.
...
??? info
    ...
    Here `*2` takes into account the actual data inflation caused by [Pipeline cutting](../developers/pipeline/) and the original data should be brought after cutting in general, so according to the worst case, the calculation here is doubled.

## Extended reading {#more-reading}

- [DataKit Log Collection Overview](datakit-logging.md)
- [Pipeline: Text Data Processing](../developers/pipeline/)
- [Pipeline debugging](../developers/pipeline/pipeline-quick-start/#debug)
