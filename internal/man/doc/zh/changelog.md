...
1. 新增 dk 采集器，移除 self 采集器(#1648)

### 问题修复 {#cl-1.11.0-fix}

1. 修复 Redis 采集器时间线冗余的问题(#1743)，完善集成测试
2. 修复 Oracle 采集器动态库安全问题(#1730)
3. 修复 DCA 服务启动失败(#1731)
4. 修复 MySQL/ElasticSearch 采集器集成测试(#1720)

### 功能优化 {#cl-1.11.0-opt}

1. 优化 etcd 采集器(#1741)
2. StatsD 采集器支持配置区分不同数据源(#1728)
3. Tomcat 采集器支持 10 及以上版本，弃用 Jolokia(#1703)
4. 容器日志采集支持配置容器内文件(#1723)
5. SQLServer 采集器指标完善和集成测试功能重构(#1694)
