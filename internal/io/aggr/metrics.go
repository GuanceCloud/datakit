...
var (
	...
	aggrFilterPoints     *prometheus.CounterVec
	aggrGeneratedBatches *prometheus.CounterVec
)
...
func init() { //nolint:gochecknoinits
	...
	aggrFilterPoints = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: "datakit",
			Subsystem: "io",
			Name:      "aggr_filter_points_total",
			Help:      "Total number of points processed by aggregator",
		},
		[]string{"type", "category"}, // type: input, selected
	)

	aggrGeneratedBatches = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: "datakit",
			Subsystem: "io",
			Name:      "aggr_generated_batches_total",
			Help:      "Total number of batches/packages generated by aggregator",
		},
		[]string{"type", "category"}, // type: metric, tail_sampling
	)
...
func recordFilterPoints(typ, category string, points int) {
	if aggrFilterPoints != nil {
		aggrFilterPoints.WithLabelValues(typ, category).Add(float64(points))
	}
}

func recordGeneratedBatches(typ, category string, count int) {
	if aggrGeneratedBatches != nil {
		aggrGeneratedBatches.WithLabelValues(typ, category).Add(float64(count))
	}
}
