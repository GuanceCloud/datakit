...
/*
Copyright 2014 The VictoriaMetrics Authors.
...
// Unmarshal unmarshal Prometheus exposition text rows from s.
...
func (rs *Rows) Unmarshal(s string) error {
	...
	rs.Rows, rs.tagsPool, err = unmarshalRows(rs.Rows[:0], s, rs.tagsPool[:0], noEscapes)
	return err
}
...
func unmarshalRows(dst []Row, s string, tagsPool []Tag, noEscapes bool) ([]Row, []Tag, error) {
	var err error
	// dstLen := len(dst)
	for len(s) > 0 {
		n := strings.IndexByte(s, '\n')
		if n < 0 {
			// The last line.
			dst, tagsPool, err = unmarshalRow(dst, s, tagsPool, noEscapes)
			if err != nil {
				return nil, nil, err
			}
			break
		}
		dst, tagsPool, err = unmarshalRow(dst, s[:n], tagsPool, noEscapes)
		if err != nil {
			return nil, nil, err
		}
		s = s[n+1:]
...
func unmarshalRow(dst []Row, s string, tagsPool []Tag, noEscapes bool) ([]Row, []Tag, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cannot unmarshal Prometheus line %q: %w", s, err)
	}
	return dst, tagsPool, nil
}
...
func (li *linesIterator) NextKey() bool {
	for {
		...
		// Do not process errors here, since they will be logged during the real data parsing later.
		li.rows, li.tagsPool, _ = unmarshalRow(li.rows[:0], li.a[0], li.tagsPool[:0], false)
		li.a = li.a[1:]
		if len(li.rows) > 0 {
