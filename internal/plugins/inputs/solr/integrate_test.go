...
func getConfAccessPoint(host, port string) []string {
	return []string{fmt.Sprintf("http://%s", net.JoinHostPort(host, port))}
}
...
func buildCases(t *testing.T) ([]*caseSpec, error) {
	...
	for _, base := range bases {
		...
		cases = append(cases, &caseSpec{
			...
			optsSearcher:     base.optsSearcher,
			optsRequestTimes: base.optsRequestTimes,
...
func (cs *caseSpec) checkPoint(pts []*point.Point) error {
	for _, pt := range pts {
		...
		switch measurement {
		case metricNameSearcher:
			opts = append(opts, cs.optsSearcher...)
			opts = append(opts, inputs.WithDoc(&SolrSearcher{}))
			...
			cs.mCount[metricNameSearcher] = struct{}{}

		case metricNameRequestTimes:
			opts = append(opts, cs.optsRequestTimes...)
			opts = append(opts, inputs.WithDoc(&SolrRequestTimes{}))
			...
			cs.mCount[metricNameRequestTimes] = struct{}{}
