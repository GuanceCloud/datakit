...
func (f *Finder) findResources(ctx context.Context, resType, path string, objs map[string]types.ObjectContent) error {
	...
	l.Debugf("Find(%s, %s) returned %d objects", resType, path, len(objs))
	return nil
}

func (f *Finder) descend(ctx context.Context, root types.ManagedObjectReference, resType string,
	tokens []property.Match, pos int, objs map[string]types.ObjectContent,
) error {
...
func init() {
	...
	addFields = map[string][]string{
		"HostSystem":   {"parent", "summary"},
		"ResourcePool": {"parent", "customValue"},
		"VirtualMachine": {
			"runtime.host", "config.guestId", "config.uuid", "runtime.powerState",
			"summary", "guest.guestId", "guest.net", "guest.hostName", "resourcePool",
			"customValue",
		},
		"Datastore":              {"parent", "info", "customValue", "summary"},
		"ClusterComputeResource": {"parent", "customValue", "summary"},
