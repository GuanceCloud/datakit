 ...
 */
//nolint:lll
const queryFmsRandom21c = `SELECT /* DK_QM_FMS */ s.con_id con_id, c.name pdb_name, s.force_matching_signature, plan_hash_value, max(dbms_lob.substr(sql_fulltext, 1000, 1)) sql_text, max(length(sql_text)) sql_text_length, max(s.sql_id) sql_id, 
...
//
//nolint:lll
const queryFmsRandom18c = `SELECT /* DK_QM_FMS */ s.con_id con_id, c.name pdb_name, s.force_matching_signature, plan_hash_value, max(dbms_lob.substr(sql_fulltext, 1000, 1)) sql_text, max(length(sql_text)) sql_text_length, max(s.sql_id) sql_id, 
 sum(parse_calls) as parse_calls,
 sum(disk_reads) as disk_reads,
 sum(direct_writes) as direct_writes,
 sum(direct_reads) as direct_reads,
...
//
//nolint:lll
const queryFmsRandom121 = `SELECT /* DK_QM_FMS */ s.con_id con_id, c.name pdb_name, s.force_matching_signature, plan_hash_value, max(dbms_lob.substr(sql_fulltext, 1000, 1)) sql_text, max(length(sql_text)) sql_text_length, max(s.sql_id) sql_id, 
 sum(parse_calls) as parse_calls,
 sum(disk_reads) as disk_reads,
 sum(direct_writes) as direct_writes,
 sum(buffer_gets) as buffer_gets,
...
// `FETCH FIRST“ doesn't exist in Oracle 11. Also, no container awareness.
//
//nolint:lll
const queryFmsRandom11 = `SELECT /* DK_QM_FMS */ 
...
//
//nolint:lll
const queryForceMatchingSignatureLastActive21c = `SELECT /* DK_QM_FMS */ s.con_id con_id, c.name pdb_name, s.force_matching_signature, plan_hash_value, 
...
//nolint:lll
const queryForceMatchingSignatureLastActive18c = `SELECT /* DK_QM_FMS */ s.con_id con_id, c.name pdb_name, s.force_matching_signature, plan_hash_value, 
 max(dbms_lob.substr(sql_fulltext, 1000, 1)) sql_text, max(length(sql_text)) sql_text_length, sq.sql_id,
 sum(parse_calls) as parse_calls,
 sum(disk_reads) as disk_reads,
 sum(direct_writes) as direct_writes,
...
//nolint:lll
const queryForceMatchingSignatureLastActive121 = `SELECT /* DK_QM_FMS */ s.con_id con_id, c.name pdb_name, s.force_matching_signature, plan_hash_value, 
 max(dbms_lob.substr(sql_fulltext, 1000, 1)) sql_text, max(length(sql_text)) sql_text_length, sq.sql_id,
 sum(parse_calls) as parse_calls,
 sum(disk_reads) as disk_reads,
 sum(direct_writes) as direct_writes,
...
//nolint:lll
const queryForceMatchingSignatureLastActive11 = `SELECT /* DK_QM_FMS */ 
	force_matching_signature, plan_hash_value, 
	sql_text, sql_text_length, sql_id,
	parse_calls,
	disk_reads,
...
//
//nolint:lll
const querySQLID21c = `SELECT /* DK_QM_SQLID */ s.con_id con_id, c.name pdb_name, sql_id, plan_hash_value, 
...
const querySQLID18c = `SELECT /* DK_QM_SQLID */ s.con_id con_id, c.name pdb_name, sql_id, plan_hash_value, 
 dbms_lob.substr(sql_fulltext, 1000, 1) sql_text, length(sql_text) sql_text_length, 
 parse_calls,
 disk_reads,
 direct_writes,
...
//nolint:lll
const querySQLID121 = `SELECT /* DK_QM_SQLID */ s.con_id con_id, c.name pdb_name, sql_id, plan_hash_value, 
 dbms_lob.substr(sql_fulltext, 1000, 1) sql_text, length(sql_text) sql_text_length, 
 parse_calls,
 disk_reads,
 direct_writes,
...
//nolint:lll
const querySQLID11 = `SELECT /* DK_QM_SQLID */ sql_id, plan_hash_value, 
 dbms_lob.substr(sql_fulltext, 1000, 1) sql_text, length(sql_text) sql_text_length, 
 parse_calls,
 disk_reads,
 direct_writes,
...
func getStatementMetricsQueries(dbVersion string) map[statementMetricsQuery]string {
	queries := make(map[statementMetricsQuery]string)
	switch {
	case isDBVersionLessThan(dbVersion, "12"):
	...
	case isDBVersionLessThan(dbVersion, "12.2"):
	...
	case isDBVersionLessThan(dbVersion, "19"):
