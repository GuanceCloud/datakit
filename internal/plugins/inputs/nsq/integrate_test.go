...
func buildCases(t *testing.T) ([]*caseSpec, error) {
	...
	bases := []struct {
		...
		exposedPorts   []string
		cmd            []string
		optsNsqTopics  []inputs.PointCheckOption
		optsNsqNodes   []inputs.PointCheckOption
	}{
		////////////////////////////////////////////////////////////////////////
		// nsq:v1.2.1
		////////////////////////////////////////////////////////////////////////
		{
			...
			exposedPorts: []string{"4161/tcp", "4151/tcp"},
			optsNsqNodes: []inputs.PointCheckOption{
	...
	for _, base := range bases {
		...
		cases = append(cases, &caseSpec{
			...
			optsNsqTopics: base.optsNsqTopics,
			optsNsqNodes:  base.optsNsqNodes,
...
func (cs *caseSpec) checkPoint(pts []*point.Point) error {
	for _, pt := range pts {
		...
		switch measurement {
		case nsqTopics:
			opts = append(opts, cs.optsNsqTopics...)
			opts = append(opts, inputs.WithDoc(&nsqTopicMeasurement{}))
			...
			cs.mCount[nsqTopics] = struct{}{}

		case nsqNodes:
			opts = append(opts, cs.optsNsqNodes...)
			opts = append(opts, inputs.WithDoc(&nsqNodesMeasurement{}))
...
func (cs *caseSpec) run() error {
	...
	cs.ipt.Lookupd = getConfAccessPoint(r.Host, cs.serverPorts[0]) // set conf URL here.
	cs.ipt.nsqdEndpointList = map[string]interface{}{
		"http://" + net.JoinHostPort(r.Host, cs.serverPorts[1]) + "/stats?format=json": nil,
	} // 4151
