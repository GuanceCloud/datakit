...
func (m *demoObj) Info() *inputs.MeasurementInfo {
	return &inputs.MeasurementInfo{
		...
		DescZh:     "用于演示 DataKit 对象类 MeasurementInfo 元数据结构的示例对象集。",
		...
		Fields: map[string]interface{}{
			...
			"some_string": &inputs.FieldInfo{
				...
				Type:     inputs.Gauge,
				Unit:     inputs.NoUnit,
				Desc:     "Example string field used to demonstrate metadata for object measurements.",
			},
			"ok": &inputs.FieldInfo{
				DataType: inputs.Bool,
				Type:     inputs.Gauge,
				Unit:     inputs.Bool,
				Desc:     "Example boolean field used to demonstrate metadata for object measurements.",
			},
		},
	}
}
