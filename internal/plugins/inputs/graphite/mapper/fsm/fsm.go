...
func (f *FSM) AddState(match string, matchMetricType string, maxPossibleTransitions int, result interface{}) int {
	...
	if matchMetricType == "" {
		...
		for _, metricType := range f.metricTypes {
			roots = append(roots, f.root.transitions[metricType])
		}
	} else {
...
func TestIfNeedBacktracking(mappings []string, orderingDisabled bool, logger *logger.Logger) bool {
	...
	for _, mapping := range mappings {
		...
		if err != nil {
			logger.Infof("Invalid match, cannot compile regex in mapping, mapping: %v, err: %v", mapping, err)
		}
	...
	for l, rules := range ruleByLength {
		...
		for i1, r1 := range rules {
			...
			for i2, r2 := range rules {
				if i2 != i1 && len(re1.FindStringSubmatchIndex(r2)) > 0 {
					...
					if !orderingDisabled && i1 < i2 {
						logger.Infof("match is a super set of match but in a lower order, the first will never be matched first_match: %v, second_match: %v",
							r1, r2)
					}
					currentRuleNeedBacktrack = false
				}
			...
			if currentRuleNeedBacktrack {
				logger.Infof("backtracking required because of match. Performance may be degraded, match: %v", r1)
