...
func (ipt *Input) GetENVDoc() []*inputs.ENVInfo {
	...
	infos := []*inputs.ENVInfo{
		...
		{FieldName: "EnableExtractK8sLabelAsTagsV2", ENVName: "ENV_INPUT_CONTAINER_EXTRACT_K8S_LABEL_AS_TAGS_V2", Type: doc.JSON, Example: `["app","name"]`, Desc: `Append the labels of the resource to the tag of the non-metric (like object and logging) data. Label keys should be specified, if there is only one key and it is an empty string (e.g. [""]), all labels will be added to the tag. The container will inherit the Pod labels. If the key of the label has the dot character, it will be changed to a horizontal line`, DescZh: `追加资源的 labels 到数据（不包括指标数据）的 tag 中。需指定 label keys，如果只有一个 key 且为空字符串（例如 [""]），会添加所有 labels 到 tag。容器会继承 Pod labels。如果 label 的 key 有 dot 字符，会将其变为横线`},
		{FieldName: "EnableExtractK8sLabelAsTagsV2ForMetric", ENVName: "ENV_INPUT_CONTAINER_EXTRACT_K8S_LABEL_AS_TAGS_V2_FOR_METRIC", Type: doc.JSON, Example: `["app","name"]`, Desc: `Append the labels of the resource to the tag of the metric data. Label keys should be specified, if there is only one key and it is an empty string (e.g. [""]), all labels will be added to the tag. The container will inherit the Pod labels. If the key of the label has the dot character, it will be changed to a horizontal line`, DescZh: `追加资源的 labels 到指标数据的 tag 中。需指定 label keys，如果只有一个 key 且为空字符串（例如 [""]），会添加所有 labels 到 tag。容器会继承 Pod labels。如果 label 的 key 有 dot 字符，会将其变为横线`},
