...
func NewPublisherMetadataStore(session EvtHandle, provider string, log *logger.Logger) (*PublisherMetadataStore, error) {
...
func NewEmptyPublisherMetadataStore(provider string, log *logger.Logger) *PublisherMetadataStore {
...
type EventMetadata struct {
	...
	EventData   []EventDataTemplate // Names of parameters from XML template.
}

type eventTemplate struct {
	Data []EventDataTemplate `xml:"data"`
}

type EventDataTemplate struct {
...
func newEventMetadataFromEventHandle(publisher *PublisherMetadata, eventHandle EvtHandle) (*EventMetadata, error) {
	xmlString, err := getEventXML(publisher, eventHandle)
	if err != nil {
		return nil, err
	}
	...
	em := &EventMetadata{
		EventID: uint16(event.EventID.ID),
		Version: uint8(event.Version),
	}
	if len(event.EventData.Pairs) > 0 {
		for _, pair := range event.EventData.Pairs {
			em.EventData = append(em.EventData, EventDataTemplate{Name: pair.Key})
		}
	} else {
...
func (em *EventMetadata) setMessage(msg string) error {
	msg = RemoveWindowsLineEndings(msg)
...
func (em *EventMetadata) equal(other *EventMetadata) bool {
	...
	eventDataNamesEqual := func(a, b []EventDataTemplate) bool {
		if len(a) != len(b) {
			return false
		}
		for n, v := range a {
