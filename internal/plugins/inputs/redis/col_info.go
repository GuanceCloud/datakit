...
func infoSkippedKeys(k, _ string) bool {
	switch k {
	case "process_id", "tcp_port", "master_replid2", "master_replid":
		return true
	default:
		return false
	}
...
func (i *instance) infoReservedTags(k, v string) bool {
	switch k {
	case "role": // master/slave
		i.role = v
		return true
	case "redis_mode": // standalone/sentinel/cluster
		i.mode = v
		return true
	case "redis_version":
		i.version = v
...
func (i *instance) parseInfoData(info string) []*point.Point {
	...
	kvs = kvs.Add("info_latency_ms", i.infoElapsed.Milliseconds())
	...
	for scanner.Scan() {
		...
		l.Debugf("redis-info: %s:%s", key, val)

		switch {
		case strings.HasPrefix(key, "errorstat_"):
			collectCache = append(collectCache, i.getErrorPoints(key, val)...)
		case strings.HasPrefix(key, "latency_percentiles_usec_"):
			collectCache = append(collectCache, i.getLatencyPoints(key, val)...)
		case val == "ok":
			kvs = kvs.Set(key, 0)
			continue
		...
		case strings.HasSuffix(val, "%"):
			perc, err := strconv.ParseFloat(val[:len(val)-1], 64)
			if err != nil {
				l.Warnf("unexpect percent on %s:%s", key, val)
			} else {
				kvs = kvs.Set(key, perc)
			}
			continue

		default:
			if i.infoReservedTags(key, val) { // is predfined tags?
				if val == "" {
					val = "unknow"
					l.Warnf("unknown tag value on %q", key)
				}
				...
				if err == nil {
					...
					case "total_system_memory":
						totalSysMem = floatval
					case "connected_slaves":
						i.connectedSlaves = int64(floatval)
					}
				} else {
					l.Debugf("redis-info: ignore %s:%v", key, val)
				}
			}
		}
	}

	// set max memory: some redis instance may not set max memory configure, we use total system instead.
	if x := kvs.Get("maxmemory"); x != nil && x.Raw().(float64) == 0.0 {
		kvs = kvs.Set("maxmemory", totalSysMem)
	}

	// update CPU usage
	var cpuUsage redisCPUUsage
	if x := kvs.Get("used_cpu_sys"); x != nil {
		cpuUsage.sys = x.GetF()
	}
	if x := kvs.Get("used_cpu_user"); x != nil {
		cpuUsage.user = x.GetF()
	}

	snapshotKey := i.mergedTags["server"]

	if prev, ok := i.infoCPULast[snapshotKey]; ok && prev != nil {
		sec := i.ipt.Interval.Seconds()
		if sec > 0 {
			sysPerc := (cpuUsage.sys - prev.sys) / sec * 100
			userPerc := (cpuUsage.user - prev.user) / sec * 100
			kvs = kvs.Set("used_cpu_sys_percent", sysPerc)
			kvs = kvs.Set("used_cpu_user_percent", userPerc)
		}
	}
	i.infoCPULast[snapshotKey] = &cpuUsage

	// NOTE: add info tags before mergedTags
	for k, v := range i.infoTags {
		kvs = kvs.AddTag(k, v)
	}
...
func (i *instance) getErrorPoints(k, v string) []*point.Point {
	...
	pts = append(pts, point.NewPoint(measurementRedisInfo, kvs, opts...))
...
// ...
func (i *instance) getLatencyPoints(k, v string) []*point.Point {
	if !i.ipt.EnableLatencyQuantile {
		return nil
	}
...
func (m *infoMeasurement) Info() *inputs.MeasurementInfo {
	return &inputs.MeasurementInfo{
		Name:   measurementRedisInfo,
		Desc:   "Redis runtime info comes from command `INFO ALL`",
		DescZh: "通过执行 `INFO ALL` 获取到的 Redis 运行时信息",
		Cat:    point.Metric,
		Fields: map[string]interface{}{
			...
			"configured_hz": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.FrequencyHz,
				Desc:     "The server's configured frequency setting.",
			},
			"lru_clock": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
			...
			"io_threads_active": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.Bool,
				Desc:     "Flag indicating if I/O threads are active.",
			},
			"shutdown_in_milliseconds": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.DurationMS,
			...
			"cluster_connections": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"client_recent_max_input_buffer": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
				Desc:     "Biggest input buffer among current client connections.",
			},
			"client_recent_max_output_buffer": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
			...
			"tracking_clients": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
				Desc:     "Number of clients being tracked (CLIENT TRACKING).",
			},
			"clients_in_timeout_table": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
				Desc:     "Number of clients in the clients timeout table.",
			},
			"total_blocking_keys": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
				Desc:     "Number of blocking keys.",
			},
			"total_blocking_keys_on_nokey": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"used_memory_peak_perc": &inputs.FieldInfo{
			...
			"used_memory_scripts": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
			...
			"mem_fragmentation_bytes": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
			...
			"allocator_frag_ratio": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Gauge,
				Unit:     inputs.PercentDecimal,
			...
			"allocator_rss_bytes": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
				Desc:     "Delta between allocator_resident and allocator_active.",
			},
			"rss_overhead_ratio": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Gauge,
				Unit:     inputs.PercentDecimal,
			...
			"lazyfree_pending_objects": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"lazyfreed_objects": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Count,
				Unit:     inputs.NCount,
			...
			"current_save_keys_processed": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
				Desc:     "Number of keys processed by the current save operation.",
			},
			"current_save_keys_total": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"rdb_last_bgsave_status": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.EnumType,
				Unit:     inputs.NoUnit,
				Desc:     "Status of the last RDB save operation (0 is `ok`, -1 is `err`).",
			},
			"rdb_last_save_time": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
			...
			"aof_rewrite_scheduled": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.Bool,
			...
			"aof_base_size": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
				Desc:     "AOF file size on latest startup or rewrite.",
			},
			"aof_pending_rewrite": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.Bool,
				Desc:     "Flag indicating an AOF rewrite operation will be scheduled once the on-going RDB save is complete.",
			...
			"loading_rdb_used_mem": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
			...
			// # Stats
			"total_connections_received": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Count,
				Unit:     inputs.NCount,
				Desc:     "Total number of connections accepted by the server.",
			},
			"total_commands_processed": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Count,
				Unit:     inputs.NCount,
				Desc:     "Total number of commands processed by the server.",
			},
			"instantaneous_ops_per_sec": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"expired_stale_perc": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Gauge,
				Unit:     inputs.Percent,
				Desc:     "The percentage of keys probably expired.",
			},
			"expired_time_cap_reached_count": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Count,
				Unit:     inputs.NCount,
				Desc:     "The count of times that active expiry cycles have stopped early.",
			},
			"expire_cycle_cpu_milliseconds": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Count,
				Unit:     inputs.DurationMS,
			...
			"pubsubshard_channels": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"repl_backlog_first_byte_offset": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"slave_read_repl_offset": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"slave_priority": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"master_sync_total_bytes": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
			...
			"master_sync_read_bytes": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.SizeByte,
			...
			"master_sync_perc": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Gauge,
				Unit:     inputs.Percent,
			...
			"master_sync_last_io_seconds_ago": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.DurationSecond,
			...
			"min_slaves_good_slaves": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.NCount,
			...
			"used_cpu_sys_main_thread": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Count,
				Unit:     inputs.DurationSecond,
				Desc:     "System CPU consumed by the Redis server main thread.",
			},
			"used_cpu_user_main_thread": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Count,
				Unit:     inputs.DurationSecond,
			...
			// # Latencystats
			"latency_percentiles_usec": &inputs.FieldInfo{
				DataType: inputs.Float,
				Type:     inputs.Gauge,
				Unit:     inputs.DurationUS,
