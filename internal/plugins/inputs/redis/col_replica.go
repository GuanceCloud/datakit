...
func (i *instance) collectReplica(ctx context.Context) {
	var (
	...
	all, err = i.doCollectReplica(ctx, i.curCli)
	if err != nil {
		l.Warnf("info.replication: %s, ignored", err.Error())
		return
	}

	if err := i.ipt.feeder.Feed(point.Metric, all,
...
func (i *instance) doCollectReplica(ctx context.Context, cc collectorClient) ([]*point.Point, error) {
	list, err := cc.info(ctx, "replication")
	if err != nil {
		return nil, err
	}

	pts, err := i.parseReplicaData(list)
	if err != nil {
		l.Warnf("parseReplicaData: %s, ignored", err.Error())
		return nil, err
	}
...
var MesurementVersionReplicaBlacklist = map[string]bool{
	"master_repl_offset":             true,
	"master_link_down_since_seconds": true,
}

func (i *instance) parseReplicaData(list string) ([]*point.Point, error) {
	...
	for scanner.Scan() {
		...
		if kvs.FieldCount() > 0 {
			...
			collectCache = append(collectCache, point.NewPoint(measureuemtRedisReplica, kvs, opts...))
		}
	}
...
func parseConnectedSlaveString(slaveName string, keyValues string) (
	id string, ip string, port string, state string, offset float64, lag float64,
) {
	// examples:
