...
func (ipt *Input) GetENVDoc() []*inputs.ENVInfo {
	...
	infos := []*inputs.ENVInfo{
		{
			FieldName: "CustomerTags",
			Type:      doc.JSON,
			Example:   "`'[\"project_id\", \"custom.tag\"]'`",
			Desc:      "Whitelist to tags",
			DescZh:    "标签白名单",
		},

		{
			FieldName: "CustomerTagsAll",
			Type:      doc.Boolean,
			Default:   `false`,
			Desc:      "extracted all attributes to tags",
			DescZh:    "提取所有标签",
		},

		{
			FieldName: "KeepRareResource",
			Type:      doc.Boolean,
			Default:   `false`,
			Desc:      "Keep rare tracing resources list switch",
			DescZh:    "保持稀有跟踪资源列表",
		},
		...
		{
			FieldName: "SplitServiceName",
			Type:      doc.Boolean,
			Default:   `false`,
			Desc:      "Get xx.system from span.Attributes to replace service name",
			DescZh:    "从 span.Attributes 中获取 xx.system 去替换服务名",
		},

		{
			FieldName: "TracingMetricEnable",
			Type:      doc.Boolean,
			Default:   `false`,
			Desc:      "These metrics capture request counts, error counts, and latency measures.",
			DescZh:    "开启请求计数，错误计数和延迟指标的采集",
		},

		{
			FieldName: "TracingMetricDisableGlobalHostTags",
			Type:      doc.Boolean,
			Default:   `false`,
			Desc:      "Disable add global host tags to OTEL metrics",
			DescZh:    "禁止追加全局主机 tag 到 OTEL 发送过来的指标数据上",
		},

		{
			FieldName: "TracingMetricTagBlacklist",
			Type:      doc.JSON,
			Example:   "`'[\"tag_a\", \"tag_b\"]'`",
			Desc:      "Blacklist of tags in the metric: `tracing_metrics`",
			DescZh:    "指标集 `tracing_metrics` 中标签的黑名单",
		},

		{
			FieldName: "TracingMetricTagWhitelist",
			Type:      doc.JSON,
			Example:   "`'[\"tag_a\", \"tag_b\"]'`",
			Desc:      "Whitelist of tags in the metric: `tracing_metrics`",
			DescZh:    "指标集 `tracing_metrics` 中标签的白名单",
		},

		{
			FieldName: "DelMessage",
			Type:      doc.Boolean,
			Default:   `false`,
			Desc:      "Delete trace message",
			DescZh:    "删除 trace 消息",
		},

		{
			FieldName: "OmitErrStatus",
			Type:      doc.JSON,
			Example:   "`'[\"404\", \"403\", \"400\"]'`",
			Desc:      "Whitelist to error status",
			DescZh:    "错误状态白名单",
		},

		{
			FieldName: "CloseResource",
			Type:      doc.JSON,
			Example:   "`'{\"service1\":[\"resource1\", \"other\"], \"service2\":[\"resource2\", \"other\"]}'`",
			Desc:      "Ignore tracing resources that service (regular)",
			DescZh:    "忽略指定服务器的 tracing（正则匹配）",
		},
...

		{
			...
			Desc:      "Total number of threads and buffer",
			DescZh:    "线程和缓存的数量",
		},

		{
			FieldName: "LocalCacheConfig",
			ENVName:   "STORAGE",
			Type:      doc.JSON,
			Example:   "`'{\"storage\":\"./otel_storage\", \"capacity\": 5120}'`",
			Desc:      "Local cache file path and size (MB) ",
			DescZh:    "本地缓存路径和大小（MB）",
		},

		{
			...
			Desc:      "HTTP agent config",
			DescZh:    "代理 HTTP 配置",
		},

		{
			FieldName: "GRPCConfig",
			ENVName:   "GRPC",
			Type:      doc.JSON,
			Example:   "`'{\"addr\": \"127.0.0.1:4317\", \"max_payload\": 16777216 }'`",
			Desc:      "GRPC agent config",
			DescZh:    "代理 GRPC 配置",
		},

		{
			FieldName: "ExpectedHeaders",
			Type:      doc.JSON,
			Example:   "`'{\"ex_version\": \"1.2.3\", \"ex_name\": \"env_resource_name\"}'`",
			Desc:      "If `expected_headers` is well config, then the obligation of sending certain wanted HTTP headers is on the client side",
			DescZh:    "配置使用客户端的 HTTP 头",
		},

		{
			FieldName: "CleanMessage",
			Type:      doc.Boolean,
			Example:   "`true/false`",
			Desc:      "Clean message generate smaller `message` field",
			DescZh:    "精简 `message` 字段大小",
		},
...
func (ipt *Input) ReadEnv(envs map[string]string) {
	...
	for _, key := range []string{
		...
		case "ENV_INPUT_OTEL_TRACING_METRIC_TAG_WHITELIST":
			var list []string
			if err := json.Unmarshal([]byte(value), &list); err != nil {
				log.Warnf("parse %s=%s failed: %s", key, value, err.Error())
			} else {
