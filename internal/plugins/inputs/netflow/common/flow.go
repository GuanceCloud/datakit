...
// Package common contains netflow commonly used stuff.
package common
...
func (f *Flow) AggregationHash() uint64 {
	h := fnv.New64()
	h.Write([]byte(f.Namespace))                           //nolint:errcheck,gosec
	h.Write(f.ExporterAddr)                                //nolint:errcheck,gosec
	h.Write(f.SrcAddr)                                     //nolint:errcheck,gosec
	h.Write(f.DstAddr)                                     //nolint:errcheck,gosec
	binary.Write(h, binary.LittleEndian, f.SrcPort)        //nolint:errcheck,gosec
	binary.Write(h, binary.LittleEndian, f.DstPort)        //nolint:errcheck,gosec
	binary.Write(h, binary.LittleEndian, f.IPProtocol)     //nolint:errcheck,gosec
	binary.Write(h, binary.LittleEndian, f.Tos)            //nolint:errcheck,gosec
	binary.Write(h, binary.LittleEndian, f.InputInterface) //nolint:errcheck,gosec
	return h.Sum64()
}
