	...
	return &inputs.MeasurementInfo{
		...
		Fields: map[string]interface{}{
			...
			"sum_timer_wait": &inputs.FieldInfo{
				...
				Desc:     "The total query execution time (picoseconds) per normalized query and schema (cumulative).",
			},
			"sum_lock_time": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Count,
				Unit:     inputs.DurationPS,
				Desc:     "The total time (picoseconds) spent waiting on locks per normalized query and schema (cumulative).",
			},
			...
			"delta_timer_wait": &inputs.FieldInfo{
				...
				Desc:     "The change in query execution time (nanoseconds) per normalized query and schema between collection intervals.",
			},
			"delta_lock_time": &inputs.FieldInfo{
				DataType: inputs.Int,
				Type:     inputs.Gauge,
				Unit:     inputs.DurationNS,
				Desc:     "The change in time (nanoseconds) spent waiting on locks per normalized query and schema between collection intervals.",
			},
