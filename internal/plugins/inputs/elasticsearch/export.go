...
func (ipt *Input) Dashboard(lang inputs.I18n) map[string]string {
	...
	case inputs.I18nZh:
		...
		return map[string]string{
			...
			"primary_deleted":           "主分片删除数量",
			"data_nodes":                "数据节点",
			"pending_tasks":             "未执行任务",
			"document":                  "文档",
			"document_primary_count":    "主分片文档数量",
			"document_count":            "文档数量",
			"document_deleted":          "删除的文档数量",
			"fs_total":                  "文件总使用量",
			"fs_available":              "文件空闲量",
			"data_node_info":            "数据节点",
			"cpu_usage":                 "cpu 使用率",
			...
			"cluster_name":              "集群名称",
			"index_name":                "索引名称",
			"index_error_status":        "错误状态索引",
			"search_current":            "当前查询",
			"search_total":              "总查询",
			"search_time":               "查询时间",
			"flush_time":                "Flush 时间",
			"flush_total":               "Flush 总数",
		}
	case inputs.I18nEn:
		//nolint:lll
		return map[string]string{
			...
			"group_cluster_overview":    "Cluster Overview",
			"group_node":                "Node",
			...
			"primary_deleted":           "primary deleted count",
			"data_nodes":                "data nodes",
			"pending_tasks":             "pending tasks",
			"document":                  "Document",
			"document_deleted":          "document deleted",
			"document_count":            "document count",
			"document_primary_count":    "document primary count",
			...
			"jvm_heap_mem_usage":        "jvm heap mem usage",
			"mem_usage":                 "mem usage",
			"mem_size":                  "mem size",
			"mem_total":                 "mem total",
			"mem_used":                  "mem used",
			...
			"index_error_status":        "index error",
...
func (ipt *Input) Monitor(lang inputs.I18n) map[string]string {
	...
	case inputs.I18nZh:
		...
		return map[string]string{
			...
			"query_time_too_long_title":           ` {{cluster_name}}-{{index_name}}查询耗时过长`,
			"query_time_too_long_message":         `>集群：{{cluster_name}} \n>索引：{{index_name}} \n>当前集群的索引{{index_name}}查询耗时{{Result}}过长，请检查！`,
			"fs_available_title":                  ` {{cluster_name}}-{{node_name}}当前文档存储空间不足`,
			"fs_available_message":                `>集群：{{cluster_name}} \n>节点：{{node_name}}\n>当前节点存储空间已不足{{Result}}GB，请扩容！`,
			"indices_lifecycle_error_title":       `ES集群{{cluster_name}}错误状态索引过多`,
			"indices_lifecycle_error_message":     `>集群：{{cluster_name}}\n>索引：{{index_name}}\n>当前集群错误状态索引过多，请检查！`,
		}
	case inputs.I18nEn:
		//nolint:lll
		return map[string]string{
			...
			"query_time_too_long_title":           `{{cluster_name}}-{{index_name}} query time is too long`,
			"query_time_too_long_message":         `>Cluster: {{cluster_name}} \n>Index: {{index_name}} \n>The query time of the current cluster index {{index_name}} is {{Result}}. Please check!`,
			"fs_available_title":                  ` {{cluster_name}}-{{node_name}} current document storage space is insufficient`,
			"fs_available_message":                `>Cluster: {{cluster_name}} \n>Node: {{node_name}}\n>The current node storage space of {{node_name}} is insufficient {{Result}}GB. Please expand!`,
			"indices_lifecycle_error_title":       `Too many error status indices in ES cluster {{cluster_name}}`,
			"indices_lifecycle_error_message":     `>Cluster: {{cluster_name}} \n>The number of error status indices in the current cluster is {{Result}}`,
		}
	default:
		return nil
	}
