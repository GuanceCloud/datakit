	...
	return &inputs.MeasurementInfo{
		...
		Fields: map[string]interface{}{
			requestTypeMap[RequestTypeAppStarted]: &inputs.FieldInfo{
				...
				Unit:     inputs.NoUnit,
				Desc:     "JSON payload from the DDTrace APM telemetry app-started request.",
			},
			requestTypeMap[RequestTypeDependenciesLoaded]: &inputs.FieldInfo{
				Type:     inputs.String,
				DataType: inputs.String,
				Unit:     inputs.NoUnit,
				Desc:     "JSON payload containing dependencies loaded by the traced application.",
			},
			requestTypeMap[RequestTypeAppClientConfigurationChange]: &inputs.FieldInfo{
				Type:     inputs.String,
				DataType: inputs.String,
				Unit:     inputs.NoUnit,
				Desc:     "JSON payload from a DDTrace APM telemetry client-configuration-change request.",
			},
			requestTypeMap[RequestTypeAppIntegrationsChange]: &inputs.FieldInfo{
				Type:     inputs.String,
				DataType: inputs.String,
				Unit:     inputs.NoUnit,
				Desc:     "JSON payload from a DDTrace APM telemetry integrations-change request.",
			},
			requestTypeMap[RequestTypeAppClosing]: &inputs.FieldInfo{
				Type:     inputs.String,
				DataType: inputs.String,
				Unit:     inputs.NoUnit,
				Desc:     "JSON payload from a DDTrace APM telemetry app-closing request.",
			},
			"spans_created": &inputs.FieldInfo{
				Type:     inputs.Count,
				DataType: inputs.Float,
				Unit:     inputs.NCount,
				Desc:     "Number of spans created by the traced application as reported by DDTrace telemetry.",
			},
			"spans_finished": &inputs.FieldInfo{
				Type:     inputs.Count,
				DataType: inputs.Float,
				Unit:     inputs.NCount,
				Desc:     "Number of spans finished by the traced application as reported by DDTrace telemetry.",
			},
		},
