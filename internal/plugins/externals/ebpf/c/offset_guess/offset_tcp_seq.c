...
int packet_tcp_header(struct __sk_buff *skb)
{
...
#pragma unroll
    ...
    {
        half_half = 0;
        bpf_probe_read(&half_half, sizeof(half_half), (__u8 *)sk + (offset_rtt - i));
        if (half_half >> 32 == pkt->seq && (half_half << 32) >> 32 == pkt->ack)
        {
            __u32 copied_seq = 0;

            __s32 cp = offset_rtt - i - 4;
...
#pragma unroll
    for (int i = 0; i < 1024; i++)
    {
        ...
        if (half_half << 32 >> 32 == win && half_half >> 32 == pkt->seq)
        {
            ...
            { // probably not
                half_half = 0;
                bpf_probe_read(&half_half, sizeof(half_half), (__u8 *)sk + (offset_rtt + i + 8));
                if (half_half << 32 >> 32 == pkt->seq || half_half >> 32 == pkt->seq)
                {
                    offset.offset_write_seq = offset_rtt + i + 4;
                    offset.state |= 0b10;
                }
            }
            else
