...
func (dec *mysqlDecPipe) Export(force bool) []*ProtoData {
	...
	for _, inf := range dec.infCache {
		if inf != nil && inf.readyToExport && inf.resource != "" {
			...
			kvs = kvs.Set(comm.FieldMysqlComment, inf.comment)
			kvs = kvs.Set(comm.FieldStatusCode, inf.responseCode)
			kvs = kvs.Set(comm.FieldErrMsg, inf.errMsg)
			kvs = kvs.Set(comm.FieldErrCode, inf.errCode)
			kvs = kvs.Set(comm.FieldStatus, code2Status(inf.errCode))
