...
- 优化日志采集中文件发现策略，增加 IONotify 机制更高效的处理新文件的发现，避免延迟采集（#2462）
- 优化主流指标采集的时间戳对齐机制，以优化时序存储效率（#2445）

### 兼容调整 {#cl-1.64.0-brk}

由于加了 API 白名单控制，老版本中一些默认开启的 API 将不再生效，需手动开启：

- 涉及 Pipeline/拨测调试有关的接口，如果不手动将这些接口加到白名单中，将导致观测云界面上的 Pipeline/拨测调试接口异常
- 已经开启 RUM 数据上传、但尚未手动将其添加到白名单，此时强制要求在 API 白名单中增加 RUM 写入接口，否则将导致 RUM 数据上传异常
...
???+ info
    ...
    在 DataKit 早期版本中，当开放非 localhost 访问时（比如将 API 监听在  `0.0.0.0:9529`），如果没有额外再手动添加 API 白名单，将有数据泄漏风险，存在严重的安全隐患。
...
- DDTrace 采集器支持收集被 Trace 服务的基本信息，将其上报到资源对象（`CO::`），其对象类型为 `traceing_service`（#2307）
