...
- Profile 支持无 Sidecar 的 Kubernetes Pod 自动发现，并按周期或 CPU/内存异常采集 Go pprof（#3220）
- Windows 的 `system` 和 `mem` 采集器新增 CPU 队列长度、可用系统页表条目数及页面读写累计数指标（#3221）
- Oracle `oracle_system` 新增 PGA 目标内存和内存上限指标 `pga_aggregate_target`、`pga_aggregate_limit`（#3231）
- Linux 和 Windows 安装脚本支持通过 `DK_ALLOW_V1_FALLBACK=1`，在系统不兼容 DataKit 2.x 时自动改装 1.x；默认不开启（#3223）

### 问题修复 {#cl-2.14.0-fix}

- 改进 DataKit 与 DCA 的连接保活和断线重连，并修复 Windows 远程升级及离线安装包处理问题（#3238）
- 修复拨测任务同步发生 panic 后重复启动变量服务、最终停止同步任务变更的问题（#3237）
- 修复 SQL Server 执行计划脱敏后 XML 命名空间声明重复的问题；脱敏失败的计划不再回退上报原文（#3233）
- 修复 SNMP 设备接口数达到 100 时接口数量显示为 0 的问题（#3230）

### 功能优化 {#cl-2.14.0-opt}

- PostgreSQL 执行计划采集文档中的 helper 创建脚本增加只读事务保护；已部署的 helper 需重新执行创建脚本才能获得此保护（#3235）
- 优化 DataKit 容器镜像构建，减小镜像体积（#3232）
...
- DDTrace 新增 `tracing_metrics.qps` 指标，按秒统计接收到的 HTTP Span，支持自定义分组标签，默认关闭（#3213）
- 新增基于 GCP Autopilot Partner 部署模式，支持 Helm 和 YAML 部署及节点侧容器指标、Pod 日志采集；仅支持 TrueWatch（#3198）

### 问题修复 {#cl-2.13.0-fix}

- 修复 SQL Server 2016 及更早版本的外键和索引查询错误，恢复表结构信息采集（#3214）
- 修复 Oracle 慢 SQL 采集因数据库与 DataKit 时区不一致导致时间游标无法更新、重复采集的问题（#3216）
- 修复 PostgreSQL 执行计划采集的多语句执行风险，并改进参数化 SQL 的识别（#3218）
- 修复拨测 SSE 长连接未随退出信号取消导致服务停止超时的问题，并支持在 systemd 服务处于失败状态时重新启动（#3219）
- 修复 macOS ARM 主机安装 amd64 DataKit 的问题，补齐 arm64 原生安装包及安装、升级支持（#3224）

### 功能优化 {#cl-2.13.0-opt}

- 新增尾采样发送失败后回退主 DataWay 的包数和点数指标，便于观察回退规模（#3217）
...
- 新增 Windows `winnetflow` 采集器，基于 ETW 采集进程级 TCP/UDP 网络流和 HTTP.sys 请求指标（#3190）
- MongoDB 采集器新增数据库监控（DBM），支持 Query Metrics、Activity 和 Slow Operations，并提供数据库过滤、命令脱敏及采集数量配置（#3201）

### 问题修复 {#cl-2.11.0-fix}

- 修复压缩尾采样包拆分前未解压以及拆分段未继承谓词摘要的问题，并兼容 DataWay 1.19.0（!4147）

### 功能优化 {#cl-2.11.0-opt}

- 调整 Pipeline 与 Aggregator 的处理顺序，使指标预聚合和尾采样使用 Pipeline 处理后的数据；被 Pipeline 或过滤器丢弃的数据不再进入聚合链路（#3202）
...
- SNMP 采集器支持采集 LLDP/CDP 拓扑关系，并在 `snmp_object` 中上报链路；新增 `device_namespace` 标签和 `interface_index` 字段（#3197）
- PostgreSQL 采集器支持 PostgreSQL 17 及以上版本的检查点指标，并保持既有 `postgresql_bgwriter` 指标字段兼容（#3200）

### 功能优化 {#cl-2.10.0-opt}

- DCA 0.1.9 可分别识别 DataKit v1 和 v2 的最新版本，避免跨主版本误判升级状态（#3195）
- 优化尾采样数据包的压缩、谓词摘要继承与大包拆分处理，需与配套 DataWay 版本联合发布（!4133）
- 完善 Kubernetes Node Label 全局主机标签及日志 `json_as_fields` 配置文档（!4136, !4143）
...
- 日志采集新增 `json_as_fields` 模式，可将 JSON 对象的顶层属性转换为日志字段（#3191）
- 浏览器拨测新增 Action，并为断言类 Action 增加轮询能力（#3193）

### 功能优化 {#cl-2.9.0-opt}

- logfwd 不再默认添加 `filename` 字段，避免转发日志携带无效文件名（#3189）
- DCA 升级至 0.1.8，优化 DataKit 列表搜索防抖及 WebSocket 连接与监听器生命周期（#3194）
- 完善 DDTrace 采集文档（!4124）
...
- RUM Session Replay 新增资源上传、存在性检查和读取 API，并限制请求体大小（#3015）
- Flameshot 新增通过 `py-spy` 采集 Python Profiling 数据的能力（#3154）
- 全局主机标签支持从 Kubernetes Node Labels 获取值（#3169）
- Flameshot OSS hprof 上传新增 `assume_role` 认证模式，可通过阿里云 STS AssumeRole 获取并刷新临时凭证（#3152）

### 问题修复 {#cl-2.8.0-fix}

- 修复 Kubernetes Pod 元数据暂时未命中缓存时移除已有容器日志 tailer 的问题（#3172）

### 功能优化 {#cl-2.8.0-opt}

- 优化尾采样大包的分片和发送流程，降低峰值内存占用（#3176）
...
- 修复自 2.1.5 版本起存在的 Container 采集器问题；该问题仅在 Pod 高频创建、删除时触发，可能增加 Kubernetes APIServer 的请求压力（#3170）
...
- 修复 SNMP 配置无效时仍提前启动 Trap 服务，导致 DataKit 重启受阻的问题（#3165）
- 浏览器拨测内置 Lightpanda 更新至 GuanceCloud 0.3.6-g1 修订版（#3167）
