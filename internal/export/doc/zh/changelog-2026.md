...
### 兼容调整 {#cl-1.94.1-compat}

- 支持全局 `measurement_version` 配置，可通过 `datakit.conf` 或安装命令参数设置指标集版本（v1/v2），统一控制 JVM/MySQL/Oracle/PostgreSQL/Redis/SQLServer 等采集器的指标集兼容模式（#3142）
