...
- DataKit 列表可分别识别 v1 和 v2 的最新版本，避免跨主版本误判升级状态。

### 0.1.8(2026/08/12) {#cl-0.1.8}

- 优化 DataKit 列表搜索防抖，避免连续输入时重复发起查询以及取消请求导致的错误提示。
- 优化 DCA WebSocket 连接和监听器生命周期，提升连接切换及异常断开场景下的稳定性。

### 0.1.7(2026/06/18) {#cl-0.1.7}

- DataKit 列表页支持工作空间搜索过滤，优化多工作空间场景下的切换体验。
- DataKit 日志页支持连接状态展示、暂停/恢复、清空、搜索高亮、日志等级过滤、自动跟随、重连和日志下载。
- 优化 DataKit 运行状况页，集中展示运行概览、基础信息、已启用采集器和采集器运行指标。
- 优化 DataKit 列表页筛选、搜索和操作交互，提升复杂筛选场景下的可用性。
- 优化重启、升级等不可用操作的提示，明确展示禁用原因。
- 修复部分 DataKit 运行信息缺失时，DCA 页面可能加载失败的问题。
- 修复部分异常场景下日志页无法进入的问题。
- 优化采集器配置、Pipeline 和升级相关操作的稳定性与安全性。
