...
- 修复 OpenTelemetry 指标采集对 count 类指标的处理，增加额外的 tag `__temporality`，便于 GuanceDB 处理（#2929）
- 修复 DDTrace 采集器自定义 tag 无效问题，该问题自 1.85.0 版本引入（#2926)

### 兼容调整 {#cl-1.87.2-brk}

- 移除了 OpenTelemetry 指标采集时的 global tag 追加（#2929）
...
- 修复 OpenTelemetry 采集器崩溃导致丢数据问题（#2924）
...
- 新增 Flameshot 用以支持动态 Profile 采集（#2876）
- Cloud meta 同步新增 Google Cloud 云主机支持（#2895）

### 问题修复 {#cl-1.87.0-fix}

- 优化 Pipeline 对 null 字符的处理（#2870）
- 修复 Redis 主从模式中 slow log 采集问题（#2895）
- 修复 Pipeline Refer table 可能崩溃的问题（#2921）
- 修复一处 logfwd service/source 字段错误问题（#2922）

### 功能优化 {#cl-1.87.0-opt}

- 优化 service 启动行为，在启动过程中如果出现报错，会输出跟 service 启动有关的上下文信息，便于错误排查（#2111）
- 优化基于 Jolokia 的 JVM 和 Kafka 指标采集（#2852）
- HTTP API 白名单功能优化，增加基于正则的白名单配置和禁用白名单的选项（#2860）
- 优化 MongoDB 采集中错误日志处理（#2916）
- 优化 Redis 采集中关于权限的说明（#2911）
- 优化 DataKit 自身指标采集配置，新增 interval 配置入口，同时，自身指标中新增 eBPF 进程有关的 CPU/内存指标采集（#2825）
- DDTrace/OpenTelemetry trace 采集中新增 Trace SDK 名称/版本号/语言/公共字段提取（#2920）
- 主机对象采集支持虚拟机/物理机检测，可据此设定不同的采集 tag（#2795）

### 兼容调整 {#cl-1.87.0-brk}

- Kafka 采集器做了较大调整，在这个版本中所有指标都归到 `kafka` 这个指标集下，且所有的指标都根据对应的 MBean 名称自动推导，不用人工配置采集的 MBean 以及其对应的指标命名（#2852）
...
- 新增 gRPC 拨测（#2862）[^2862]
- logfwd 采集配置支持通过 CRD 热更新（#2858）

[^2862]: 目前 Studio 尚不支持新建 gRPC 拨测任务，暂时只支持通过 DataKit 本地配置拨测任务。

### 问题修复 {#cl-1.86.0-fix}

- 修复日志采集 CRD 中 `PodTargetLabel` 无效问题（#2880）
- 修复 cgroup 在 Ubuntu 下 CPU 使用率限制失效问题（#2894）
- 修复 profile 指标生成中 tag 设置问题（!3757）
...
- 修复 Profile 采集器磁盘缓存在容器部署形态下可能导致失败的问题。同时将其磁盘缓存 worker 默认值降低到 1 个（#2885/#2881）
- 修复拨测采集器内网无法拨测问题，该 bug 自 1.84.0 引入（#2887）
- 修复 *datakit.service* 文件中 CPU 限额配置错误问题（#2888）
- 修复容器日志过滤优先级错误的问题，该问题自 1.85.0 版本引入（#2890）
- 开始主动采集 cronjob/job 类容器的日志，不再忽略这类容器的日志采集（#2891）
...
- DCA 服务端新增 Prometheus 指标暴露，同时新增其内置视图（#2638）
- SNMP 采集新增基于 LLDP 的网络邻居嗅探数据采集（#2829）

### 问题修复 {#cl-1.85.0-fix}

- 修复默认采集器配置文件生成问题，该问题自 1.83.0 版本引入（#2867）
- 修复 OpenTelemetry 采集中无效字段的处理（#2869）
- 修复 SNMP 采集器在 Windows 下无法找到 profile 的问题（#2877）

### 功能优化 {#cl-1.85.0-opt}

- Kubernetes 变更有关事件新增 `change_id` 字段（#2866）
- 优化 DataKit 自身内存占用（#2862）
- PostgreSQL 新增数据库自发现功能以及 function 有关指标采集（#2856）
- DataKit 增加 Deployment 部署文档说明（#2617）
- Redis 采集器补全低版本上的 `maxclients` 指标采集（#2873）
- 日志采集中新增保护措施，避免文件被 DataKit 长期占据无法删除（#2874）
- 优化 eBPF 采集的内存占用（#2875）
- 其它文档更新（#2843）

### 兼容调整 {#cl-1.85.0-brk}

- OpenTelemetry 采集中移除 HTTP JSON body 支持，目前其 HTTP 协议只支持 Protobuf body（#2869）
- 移除采集器级别的集群选举功能（即通过 DataKit operator 来协调采集器在各个 DataKit 实例上的采集行为），只保留中心选举功能（#2872）
...
- 修复 Redis 采集器 `up` 字标问题（#2863）
- Kubernetes 对象采集时，移除其 message 中一些噪音数据（#2859）
- 修复 `promv2` 采集器中可能存在的泄露问题（#2864）
- 修复容器日志非挂载采集时通配路径问题（#2861）
...
- 修复 OpenTelemetry 字段配置拼写错误（#2786）
- 修复 Profiling 上报时错误添加了分流标志的问题（#2786）
    - 该问题在高版本 DataWay（>=1.8.0）上会被当做分流请求处理，导致数据丢失
