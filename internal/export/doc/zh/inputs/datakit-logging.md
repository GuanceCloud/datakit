...
> 如果希望 DataKit 上只开启单个（或少数几个）TCP 端口，那么需要在后续 [Pipeline](../developers/pipeline/index.md) 处理中，对切割出来的字段，识别其特征，并通过函数 [`set_tag()`](../developers/pipeline/pipeline-built-in-function.md#fn-set-tag) 来标记其 `service`（目前无法修改日志的 `source` 字段，且该功能只有 [1.2.8 以上的版本才支持](../datakit/changelog.md#cl-1.2.8)）。
