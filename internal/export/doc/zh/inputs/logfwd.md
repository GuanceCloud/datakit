...
| `json_as_fields`                | boolean | 否       | 将 JSON 根对象转换为日志字段，默认 `false`，规则见 [JSON 字段模式](logging.md#json-as-fields)          | `false`                   |
...
`json_as_fields` 当前仅支持 `LOGFWD_LOG_CONFIGS` 手动配置。ClusterLoggingConfig CRD 暂不支持该字段；logfwd 客户端和 DataKit 服务端均升级后字段模式才会生效。
