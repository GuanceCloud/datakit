...
=== "`datakit.conf`"
    ...
    After the configuration is complete, you can use the `curl` command to test whether the configuration is successful:

    ```shell
    sudo curl --no-buffer -XGET --unix-socket /tmp/datakit.sock http:/localhost/v1/ping
    ```

    For more information on the test commands for `curl`, see [here](https://superuser.com/a/925610){:target="_blank"}.
    ...
    ```toml
    [http_api]
      request_rate_limit       = 100.0 # Default: 100
    ...
    ???+ warning "Configure Rate Limiting Parameters Properly"
        ...
        - Set `request_rate_limit` to 0 to disable API rate limiting
        - If you indeed need to handle a large volume of data writes, increase the limit appropriately to avoid data loss
        - `request_rate_limit_burst` can be set to 0 to disable burst traffic
        - `request_rate_limit_ttl` should not be set too large; a larger TTL will consume more DataKit memory. If set too small, it means more burst requests are allowed per unit time

    ???+ tips "Check Rate Limiting Status"

        If APIs are rate-limited (e.g., for traces, logs, etc.), it will cause collected data loss. You can view the corresponding rate-limiting status in the "HTTP APIs" panel of the DataKit Monitor (`datakit monitor -V`) (the Status column shows `Too Many Requests`). Additionally, you can query the following metrics in DataKit's built-in metrics to check for rate limiting:

        ```shell
        # If DataKit is deployed within container/Kubernetes, enter the corresponding container first.
        curl -s http://localhost:9529/metrics | grep -a datakit_http_api_total
        ```

        An example of the output is as follows, where `api` refers to the specific request route (different for trace/log collection), and `status` refers to the HTTP status code:
    ...
    For security purposes, DataKit restricts access to some of its own APIs by default (these APIs can only be accessed via localhost). If DataKit is deployed in a public network environment and you need to access these APIs from other machines or the public network, modify the `public_apis` field configuration in *datakit.conf* as follows:

    ```toml
    [http_api]
      public_apis = [
        # Allow access to DataKit's built-in metric exposure endpoint /metrics
        "/metrics",
        # ... Other endpoints
      ]
    ```

    By default, `public_apis` is empty. For convenience and compatibility, only [some endpoints](apis.md) are open by default, and all other endpoints block external access. For collector-specific endpoints (e.g., trace collectors), access is automatically enabled once the collector is activated, allowing external access by default.

    ???+ warning

        Once `public_apis` is not empty, you need to manually re-add the APIs that are enabled by default:
