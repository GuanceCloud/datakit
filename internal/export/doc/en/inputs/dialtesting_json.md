...
#### BROWSER Dial Test {#browser}

BROWSER dial testing runs browser scripts through the embedded browser runner and reports the result as the `browser_dial_testing` metric. The `browser_config` field is a YAML string.
...
##### `browser_config` Definition {#browser-config}

`browser_config` is a YAML string. Common fields include:

| Field        | Type   | Whether Required | Description                                      |
| :---         | ---    | ---      | ---                                       |
| `name`       | string | N        | Browser script name                            |
| `target`     | string | N        | Default target URL, used when a `goto` step does not specify a URL    |
| `timeout_ms` | int    | N        | Browser dial timeout in milliseconds            |
