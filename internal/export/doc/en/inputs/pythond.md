...
General event field description:
 ...
 | ----              | ----                           | ----            | ----                                                                      |
 | `df_date_range`   | Integer                        | Y               | Time range. Unit s                                                        |
 | `df_source`       | String                         | Y               | Data source, value `system` , `monitor` , `user`                          |
 | `df_status`       | Enum                           | Y               | Status, value `ok` , `info` , `warning` , `error` , `critical` , `nodata` |
 | `df_event_id`     | String                         | Y               | event ID                                                                  |
 | `df_title`        | String                         | Y               | Title                                                                     |
 | `df_message`      | String                         | N               | Description of the event details                                          |
 | `other fields...` | `kwargs`, such as `k1=5, k2=6` |                 | Other extra field                                                         |

- When `df_source = monitor`:

Represent an event generated by <<<custom_key.brand_name>>> detection function, with the following additional fields:
...
### How to troubleshoot errors {#troubleshoot}
...
1. If there is no problem with '1', check if all the modules imported in the Python script are installed;
