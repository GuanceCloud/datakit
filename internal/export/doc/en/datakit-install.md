...
### Using Kubernetes Secret to Secure DataWay Token {#secure-dataway}
<!-- markdownlint-enable -->
DataKit supports two methods to secure the `dataway_token` in Kubernetes configurations:

- When installing DataKit using Helm, you can hide the `dataway_token` by configuring Secret:

    **Install using Helm command with Secret mode enabled**
    ...
    - The `ENV_DATAWAY` environment variable in the Pod references from the Secret

- When installing using native YAML files, you need to manually create the Secret and modify the environment variable reference:
    ...
    1. **Create Secret**
        ...
        Encode your `dataway_url` using base64:
