...
// Package labels is from the Kubernetes repo, with the Requirement functionality extended to support Glob patterns.
package labels
...
type Requirement struct {
	...
	strValues []string
	// Equals, DoubleEquals, and NotEquals supported Glob.
...
func NewRequirement(key string, op selection.Operator, vals []string, opts ...field.PathOption) (*Requirement, error) {
	...
	var globMatcher glob.Glob

	switch op { //nolint:exhaustive
	case selection.Equals, selection.DoubleEquals, selection.NotEquals:
		// That are support Glob, so do not validate.
		if len(vals) != 1 {
			allErrs = append(allErrs, field.Invalid(valuePath, vals, "for '=', '==', '!=' operators, exactly one value is required"))
		} else {
			var err error
			globMatcher, err = glob.Compile(vals[0])
			if err != nil {
				allErrs = append(allErrs, field.Invalid(valuePath, vals[0], err.Error()))
			}
		}
	...
	return &Requirement{key: key, operator: op, strValues: vals, globMatcher: globMatcher}, allErrs.ToAggregate()
}
...
func (r *Requirement) matchGlob(value string) bool {
	if r.globMatcher != nil {
		return r.globMatcher.Match(value)
	}
	return false
}
...
func (r *Requirement) Matches(ls Labels) bool {
	switch r.operator { //nolint:exhaustive
	case selection.In:
		if !ls.Has(r.key) {
			return false
		}
	...
	case selection.Equals, selection.DoubleEquals:
		...
		return r.matchGlob(ls.Get(r.key))
...
func (in *Requirement) DeepCopy() *Requirement { // nolint:stylecheck
	if in == nil {
		return nil
	}
	out := new(Requirement)
...
func (s internalSelector) RequiresExactMatch(label string) (value string, found bool) {
	for ix := range s {
		if s[ix].key == label {
			switch s[ix].operator { //nolint:exhaustive
...
IdentifierLoop:
	for {
		switch ch := l.read(); { //nolint:exhaustive
		case ch == 0:
...
func (l *Lexer) Lex() (tok Token, lit string) {
	switch ch := l.skipWhiteSpaces(l.read()); { //nolint:exhaustive
	case ch == 0:
...
func parse(selector string, path *field.Path) (internalSelector, error) {
	...
	return internalSelector(items), err //nolint:unconvert
}
...
func ValidatedSelectorFromSet(ls Set) (Selector, error) {
	if ls == nil || len(ls) == 0 { //nolint:gosimple
		return internalSelector{}, nil
	}
