...
func (app *monitorAPP) setupFlex() {
	...
	if app.verbose { // with -V, we show more stats info
		app.flex.SetDirection(tview.FlexRow).
			...
			AddItem(tview.NewFlex().SetDirection(tview.FlexColumn). // filter related stats
				...
				0, 10, false).
			AddItem(tview.NewFlex().SetDirection(tview.FlexColumn).
				AddItem(app.plStatTable, 0, 10, false).  // pipeline stats
				AddItem(app.walStatTable, 0, 10, false), // WAL stats
				0, 10, false).
			AddItem(tview.NewFlex().SetDirection(tview.FlexColumn).
				AddItem(app.dwptsTable, 0, 10, false).
				AddItem(app.dwTable, 0, 10, false),
				0, 10, false).
	...
	if len(app.onlyModules) > 0 {
		...
		if exitsStr(app.onlyModules, moduleIO) {
			flex.AddItem(tview.NewFlex().SetDirection(tview.FlexColumn).AddItem(app.dwptsTable, 0, 10, false), 0, 10, false)
		}
		...
		if exitsStr(app.onlyModules, moduleWAL) {
			flex.AddItem(tview.NewFlex().SetDirection(tview.FlexColumn).AddItem(app.walStatTable, 0, 10, false), 0, 10, false)
		}
