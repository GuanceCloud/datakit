...
func UserAgentHandle(str string) (map[string]interface{}, map[string]ast.DType) {
	...
	retType := make(map[string]ast.DType)

	ua := user_agent.New(str)

	res["isMobile"] = ua.Mobile()
	retType["isMobile"] = ast.Bool

	res["isBot"] = ua.Bot()
	retType["isBot"] = ast.Bool

	res["os"] = ua.OS()
	retType["os"] = ast.String

	name, version := ua.Browser()
	res["browser"] = name
	retType["browser"] = ast.String

	res["browserVer"] = version
	retType["browserVer"] = ast.String

	en, v := ua.Engine()
	res["engine"] = en
	retType["engine"] = ast.String
	res["engineVer"] = v
	retType["engineVer"] = ast.String
	res["ua"] = ua.Platform()
	retType["ua"] = ast.String
	return res, retType
}
